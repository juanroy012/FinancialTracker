...
def account_create(
    ...
    try:
        account = create_account(new, current_user["id"], conn)
...
def remove_account(
    ...
    if delete_account(account_id, current_user["id"], conn):
        return None
...
def edit_account(
    ...
    updated = update_account(account_id, updated_account, current_user["id"], conn)
    if updated is None:
